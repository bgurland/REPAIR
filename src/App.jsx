...
// ─── PROLAPSE SECTION (accordion replaces tabs) ───────────────────────────────
...
const IMPACTSurvey = ({ onComplete }) => {
  ...
  // Show "early report" button after step 8 (core questions done)
  ...
  return (
    <div>
      ...
      {/* Show early report option from step 9 onward */}
...
// ─── CALCULATORS SECTION (vertical stepper) ───────────────────────────────────
          ...
          {step === 3 && (
            <div>
              ...
              <Card style={{ background: C.tealLight, borderRadius: 14, padding: 14, marginTop: 4 }}>
                ...
                <div style={{ color: C.navy, fontSize: 16, lineHeight: 1.6 }}>{surgRisk.length === 0 ? "Reassuring — your team will do their own full assessment." : surgRisk.length <= 2 ? "Worth discussing with your surgical team so they can plan accordingly." : "Your surgeon will want to know about these and may recommend additional pre-operative preparation."}</div>
              </Card>
...
const SurgicalSection = ({ speak, stop, speaking }) => {
  ...
  const sections = [
    ...
    { id: "happen", q: "What will happen to my symptoms?", content: (<div><Callout icon="⭐" color={C.coral} bg={C.coralLt} title="The most important thing to understand" body="Fixing the anatomy does not guarantee fixing the function. This is something your surgical team should discuss with you openly before any procedure." />{[{ title: "Fecal Incontinence & Leakage", icon: "💧", good: true, body: "Surgery for rectal prolapse is more likely to improve leakage and incontinence. If this is your most bothersome symptom, the evidence is more reassuring." }, { title: "Constipation & Difficult Evacuation", icon: "🚽", good: false, body: "Improvement is less predictable — around 60–70% of patients see improvement, meaning 30–40% may not. If straining is your primary complaint, discuss this honestly with your surgeon first." }].map(s => (<Card key={s.title} style={{ border: `2px solid ${s.good ? C.teal : C.warn}`, borderRadius: 14, padding: 14, marginBottom: 12 }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}><span style={{ fontSize: 22 }}>{s.icon}</span><span style={{ color: C.navy, fontWeight: 700, fontSize: 16, flex: 1 }}>{s.title}</span><span style={{ background: s.good ? C.tealLight : C.warnLt, color: s.good ? C.teal : C.warn, borderRadius: 20, padding: "3px 10px", fontSize: 13, fontWeight: 700 }}>{s.good ? "More predictable" : "Less predictable"}</span></div><div style={{ color: C.navy, fontSize: 16, lineHeight: 1.7 }}>{s.body}</div></Card>))}<Callout icon="💙" body="If surgery doesn't resolve everything, that is not a failure. Pelvic floor PT, lifestyle changes, and follow-up care all remain part of the picture." /></div>) },
    { id: "mesh", q: "Will I need mesh?", content: (<div><Callout body="Not all rectopexy procedures use mesh. Your surgeon will explain what they recommend and why. The information below supports an informed conversation." icon="ℹ️" />{[{ type: "Synthetic Mesh", icon: "🔩", desc: "Permanent synthetic material (polypropylene). Durable and widely used.", erosion: "~1.8% erosion rate", rec: "~3.7% recurrence" }, { type: "Biologic Mesh", icon: "🧬", desc: "Processed animal or human tissue. Gradually absorbed over time.", erosion: "~0.7% erosion rate", rec: "~4.0% recurrence" }, { type: "Suture Only — No Mesh", icon: "🪡", desc: "Rectum secured with sutures alone. A valid option for many patients.", erosion: "No mesh — no erosion risk", rec: "Comparable outcomes" }].map(m => (<Card key={m.type} style={{ borderRadius: 14, padding: 14, marginBottom: 10 }}><div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 8 }}><span style={{ fontSize: 22 }}>{m.icon}</span><span style={{ color: C.navy, fontWeight: 700, fontSize: 16 }}>{m.type}</span></div><div style={{ color: C.navy, fontSize: 16, lineHeight: 1.6, marginBottom: 10 }}>{m.desc}</div><div style={{ display: "flex", gap: 24 }}><div><div style={{ color: C.muted, fontSize: 13, textTransform: "uppercase" }}>Erosion</div><div style={{ color: C.navy, fontSize: 15, fontWeight: 600 }}>{m.erosion}</div></div><div><div style={{ color: C.muted, fontSize: 13, textTransform: "uppercase" }}>Recurrence</div><div style={{ color: C.navy, fontSize: 15, fontWeight: 600 }}>{m.rec}</div></div></div></Card>))}<div style={{ color: C.navy, fontWeight: 700, fontSize: 16, marginBottom: 10 }}>My preference to discuss with my surgeon:</div>{[{ id: "discuss", label: "I'd like to discuss mesh options" }, { id: "avoid", label: "I'd prefer to discuss suture rectopexy (no mesh)" }, { id: "open", label: "I'm open to whatever my surgeon recommends" }].map(o => (<OptionCard key={o.id} label={o.label} selected={meshPref === o.id} onClick={() => setMeshPref(o.id)} />))}</div>) },
    { id: "risk", q: "Am I a good candidate for surgery?", content: (<div><div style={{ color: C.slate, fontSize: 16, lineHeight: 1.6, marginBottom: 16 }}>Select any factors that apply to you. This helps you start an informed conversation with your surgeon about surgical risk.</div>{SURG_RISKS.map(r => (<CheckCard key={r} label={r} checked={surgRisk.includes(r)} onClick={() => setSurgRisk(prev => prev.includes(r) ? prev.filter(x => x !== r) : [...prev, r])} />))}<Card style={{ background: C.tealLight, borderRadius: 14, padding: 14, marginTop: 4 }}><div style={{ color: C.teal, fontWeight: 700, fontSize: 16, marginBottom: 4 }}>{surgRisk.length === 0 ? "✅ No risk factors identified" : `${surgRisk.length} factor${surgRisk.length > 1 ? "s" : ""} to discuss`}</div><div style={{ color: C.navy, fontSize: 16, lineHeight: 1.6 }}>{surgRisk.length === 0 ? "Reassuring — your team will do their own full pre-op assessment." : "Bring these to your surgical consultation. Your team can explain how they affect your individual risk."}</div></Card></div>) },
    { id: "recurrence", q: "Could prolapse come back after surgery?", content: (<div><Callout body="Yes — rectal prolapse can recur after surgery. Knowing your risk factors helps you and your surgeon make the best plan. Some factors can be improved before surgery." icon="🔄" />{REC_RISKS.map(r => (<div key={r.id}><CheckCard label={r.label} checked={recRisk.includes(r.id)} onClick={() => setRecRisk(prev => prev.includes(r.id) ? prev.filter(x => x !== r.id) : [...prev, r.id])} color={C.coral} />{recRisk.includes(r.id) && (<div style={{ background: C.coralLt, borderRadius: 12, padding: "12px 16px", marginTop: -6, marginBottom: 10 }}><div style={{ color: C.slate, fontSize: 15, lineHeight: 1.6 }}>{r.why}</div></div>)}</div>))}{recRisk.length > 0 && (<Card style={{ background: C.tealLight, borderRadius: 14, padding: 14 }}><div style={{ color: C.teal, fontWeight: 700, fontSize: 16, marginBottom: 6 }}>{recRisk.length} factor{recRisk.length > 1 ? "s" : ""} identified</div><div style={{ color: C.navy, fontSize: 16, lineHeight: 1.6 }}>Bring this to your appointment. The more your team knows, the better they can plan with you.</div></Card>)}</div>) },
    {
      id: "questions", q: "What should I ask my surgeon?", content: (
        <div>
  ...
  const send = async (text) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ system: SYSTEM_PROMPT + buildContext(), messages: newMessages }),
      });
