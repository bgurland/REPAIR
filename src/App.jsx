...
const HomeSection = ({ onNav }) => (
  <div>
    <div className="rounded-2xl p-6 mb-6 text-center" style={{ background: `linear-gradient(135deg, ${C.teal}, ${C.navyMid})` }}>
      ...
      <h1 style={{ color: "#fff", fontSize: 26, fontWeight: 800, marginBottom: 4 }}>RENEW</h1>
...
const ProlapseSection = () => {
  ...
  return (
    <div>
      ...
      {tab === "what" && (
        <div>
          <p style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>
          ...
          <p style={{ color: C.navy, fontSize: 15, fontWeight: 700, marginBottom: 8 }}>▶ Watch this animation first</p>
          <VimeoEmbed videoId="743819969" title="Rectal Prolapse & Rectocele" />
          <Card>
            <div style={{ color: C.teal, fontWeight: 700, fontSize: 15, marginBottom: 8 }}>Internal vs External</div>
            <div style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 12 }}>
            ...
            <div style={{ color: C.navy, fontSize: 14, lineHeight: 1.7 }}>
      ...
      {tab === "grading" && (
        <div>
          <p style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 12 }}>Rectal prolapse is graded by how far the rectum descends. Understanding your grade helps your team choose the right approach.</p>
      ...
      {tab === "eval" && (
        <div>
          <p style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>A healthcare provider will typically take a thorough approach to evaluation:</p>
          {[
      ...
      {tab === "treatment" && (
        <div>
          ...
          <p style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 12 }}>General treatment categories (from conservative to surgical):</p>
          {[
...
const LifestyleSection = () => {
  ...
  return (
    <div>
      ...
      <div style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>
      ...
      <div style={{ color: C.navy, fontSize: 15, fontWeight: 700, marginBottom: 12, marginTop: 8 }}>The 6 Lifestyle Medicine Pillars</div>
...
// ─── CALCULATORS ─────────────────────────────────────────────────────────────
const WEXNER_QS = [
  { label: "Solid stool leakage", key: "solid" },
  { label: "Liquid stool leakage", key: "liquid" },
  { label: "Gas / flatus leakage", key: "gas" },
  { label: "Wear a pad or protective liner", key: "pad" },
  { label: "Avoid activities due to incontinence", key: "lifestyle" },
];
const FREQ_OPTS = [
  { label: "Never", value: 0 },
  { label: "Rarely (< 1×/month)", value: 1 },
  { label: "Sometimes (≥ 1×/month, < 1×/week)", value: 2 },
  { label: "Usually (≥ 1×/week, < 1×/day)", value: 3 },
  { label: "Always (≥ 1×/day)", value: 4 },
];

const ODS_QS = [
  { label: "How often do you have a bowel movement?", key: "freq", opts: [{ label: "Every 1–2 days", value: 0 }, { label: "Every 3–4 days", value: 2 }, { label: "Once per week", value: 4 }, { label: "Less than once per week", value: 6 }] },
  { label: "Do you strain during defecation?", key: "strain", opts: [{ label: "Never", value: 0 }, { label: "Rarely", value: 2 }, { label: "Sometimes", value: 4 }, { label: "Usually", value: 6 }, { label: "Always", value: 8 }] },
  { label: "Do you feel of incomplete evacuation?", key: "incomplete", opts: [{ label: "Never", value: 0 }, { label: "Rarely", value: 1 }, { label: "Sometimes", value: 2 }, { label: "Usually", value: 3 }, { label: "Always", value: 4 }] },
  { label: "Do you use laxatives or enemas?", key: "laxatives", opts: [{ label: "Never", value: 0 }, { label: "Rarely", value: 1 }, { label: "Sometimes", value: 2 }, { label: "Usually", value: 3 }, { label: "Always", value: 4 }] },
  { label: "Do you use manual assistance (digitation)?", key: "digitation", opts: [{ label: "Never", value: 0 }, { label: "Rarely", value: 1 }, { label: "Sometimes", value: 2 }, { label: "Usually", value: 3 }, { label: "Always", value: 4 }] },
  { label: "Do you have abdominal or perineal pain?", key: "pain", opts: [{ label: "Never", value: 0 }, { label: "Rarely", value: 1 }, { label: "Sometimes", value: 2 }, { label: "Usually", value: 3 }, { label: "Always", value: 4 }] },
  { label: "Has this decreased your quality of life?", key: "qol", opts: [{ label: "No", value: 0 }, { label: "Yes", value: 1 }] },
];

const wexnerBand = (s) => s === 0 ? { label: "No incontinence", color: C.teal } : s <= 4 ? { label: "Mild incontinence", color: C.teal } : s <= 8 ? { label: "Moderate incontinence", color: C.warn } : s <= 14 ? { label: "Severe incontinence", color: C.coral } : { label: "Very severe incontinence", color: C.red };
const wexnerMsg = (s) => s === 0 ? "Your responses suggest you are not experiencing fecal incontinence at this time." : s <= 4 ? "Your responses suggest mild fecal incontinence. Even mild leakage is worth discussing with your healthcare team — there are effective options." : s <= 8 ? "Your responses suggest moderate fecal incontinence that may be affecting your daily life. Please share this score with your team — a range of treatments can make a real difference." : "Your responses suggest significant fecal incontinence. You don't have to live with this. Effective treatments exist and you deserve support.";

const odsBand = (s) => s <= 8 ? { label: "Mild / absent ODS", color: C.teal } : s <= 15 ? { label: "Moderate ODS", color: C.warn } : s <= 23 ? { label: "Severe ODS", color: C.coral } : { label: "Very severe ODS", color: C.red };
const odsMsg = (s) => s <= 8 ? "Your responses suggest mild or minimal obstructed defecation symptoms." : s <= 15 ? "Your responses suggest moderate difficulty with evacuation. Many people find relief with pelvic floor PT, dietary changes, and bowel retraining." : "Your responses suggest significant difficulty with bowel evacuation. Please share this score with your healthcare team — a comprehensive evaluation can help identify the right approach.";

const CalculatorShell = ({ title, total, max, children, onDone }) => (
  <div>
...
const QuestionRow = ({ label, value, onChange, opts = FREQ_OPTS }) => (
  <Card>
    <div style={{ color: C.navy, fontSize: 14, fontWeight: 600, marginBottom: 10 }}>{label}</div>
    {opts.map(o => (
      <button key={o.value} onClick={() => onChange(o.value)}
        className="w-full text-left rounded-xl px-3 py-2 mb-2 flex items-center gap-2 transition-all"
        style={{ background: value === o.value ? C.teal : C.tealLight, border: "none", minHeight: 44 }}>
        <div className="rounded-full flex-shrink-0" style={{ width: 18, height: 18, border: `2px solid ${value === o.value ? "#fff" : C.teal}`, background: value === o.value ? "#fff" : "transparent" }} />
        <span style={{ color: value === o.value ? "#fff" : C.navy, fontSize: 13 }}>{o.label}</span>
      </button>
    ))}
  </Card>
);
...
const CalculatorsSection = ({ scores, setScores, primarySymptom, setPrimarySymptom }) => {
  ...
  const wTotal = Object.values(wAnswers).reduce((a, b) => a + b, 0);
  const oTotal = Object.values(oAnswers).reduce((a, b) => a + b, 0);
  ...
  const tabs = [
    ...
    { id: "wexner", label: "Wexner (FI)" },
    { id: "ods", label: "ODS Score" },
  ...
  return (
    <div>
      ...
      <Callout icon="📋" body="Complete these questionnaires before your appointment. The scores give your healthcare team a clear, objective picture of your symptom burden — and help you have a more productive conversation." />
        ...
        <div>
          ...
          {primarySymptom && (
            <Callout icon="💡" title="Why this matters" color={C.teal} bg={C.tealLight} body={
              ...
              : "You're experiencing several significant symptoms. The scoring tools below will help quantify each one — bringing these results to your appointment gives your team a clear picture."
            } />
          )}
        </div>
      )}
      ...
      {active === "wexner" && (
        <div>
          <Callout body="Fecal incontinence affects approximately 1 in 3 people seen in primary care — yet most never bring it up. Completing this questionnaire is a courageous first step." icon="💙" />
          <CalculatorShell title="Wexner Fecal Incontinence Score" total={wTotal} max={20}>
            {WEXNER_QS.map(q => (
              <QuestionRow key={q.key} label={q.label} value={wAnswers[q.key]} onChange={v => setWAnswers({ ...wAnswers, [q.key]: v })} />
            ))}
          </CalculatorShell>
          {Object.keys(wAnswers).length === 5 && (
            <ScoreResult label="Wexner FI Score" score={wTotal} max={20} band={wexnerBand(wTotal)} msg={wexnerMsg(wTotal)} />
          )}
        </div>
      )}
      ...
      {active === "ods" && (
        <div>
          <Callout body="Obstructed defecation affects quality of life significantly — and is often undertreated. Your answers help quantify what you're experiencing." icon="💙" />
          <CalculatorShell title="ODS Score (Obstructed Defecation Syndrome)" total={oTotal} max={31}>
            {ODS_QS.map(q => (
              <QuestionRow key={q.key} label={q.label} value={oAnswers[q.key]} onChange={v => setOAnswers({ ...oAnswers, [q.key]: v })} opts={q.opts} />
            ))}
          </CalculatorShell>
          {Object.keys(oAnswers).length === 7 && (
            <div>
              <ScoreResult label="ODS Score" score={oTotal} max={31} band={odsBand(oTotal)} msg={odsMsg(oTotal)} />
              {oTotal > 9 && <VimeoEmbed videoId="1175949349" title="Constipation & Dyssynergic Defecation" />}
            </div>
          )}
        </div>
      )}
    ...
    <div>
      ...
      {tab === "rectopexy" && (
        <div>
          ...
          <p style={{ color: C.navy, fontSize: 15, fontWeight: 700, marginBottom: 8 }}>▶ Watch this animation</p>
      ...
      {tab === "mesh" && (
        <div>
          ...
          <div style={{ color: C.navy, fontWeight: 700, fontSize: 14, marginBottom: 8 }}>My mesh preference for discussion:</div>
          {[
      ...
      {tab === "questions" && (
        <div>
          <div style={{ color: C.navy, fontSize: 14, lineHeight: 1.7, marginBottom: 16 }}>Empowered patients get better care. Use this list to prepare for your surgical consultation:</div>
          {[
...
// Do not edit without updating the REPAIR Chatbot Development Log

const SYSTEM_PROMPT_BASE = `You are a warm, knowledgeable patient education assistant embedded in the REPAIR app — a free, clinician-authored tool for patients with rectal prolapse, bowel dysfunction, and pelvic floor disorders. The app was created by a colorectal surgeon and lifestyle medicine physician at Stanford.
...
Provider credibility: If a patient questions their provider's competence, do not validate criticism or take sides. Acknowledge that clinical opinions sometimes differ: "Surgeons sometimes disagree on this — it's worth asking your provider to explain the evidence behind their recommendation."
...
// Track 1 addition — injected when listeningStyle === "depth"
...
// Red flag detection — V2: context-aware
// Informational context keywords indicate the patient is discussing a risk, not experiencing an emergency
const RED_FLAG_KEYWORDS = ["bleeding", "blood", "can't push back", "stuck outside", "severe pain", "fever", "emergency", "purple", "dark", "not going back", "cannot pass stool", "can't pass stool"];
const INFORMATIONAL_CONTEXT = ["surgeon mentioned", "doctor mentioned", "told me", "risk of", "talking about", "discussed", "mentioned the risk", "said there was a risk", "my doctor said", "they said", "provider said"];
...
const Chatbot = ({ appState, onClose }) => {
  ...
  // Determine if the style check-in should be appended to a response
  const shouldAskStyle = (userMessage, botReply) => {
    if (hasAskedStyle || listeningStyle) return false;
    const EMOTIONAL_KEYWORDS = ["scared", "worried", "afraid", "anxious", "will i ever", "normal", "alone", "nervous", "upset", "fear", "frustrated", "don't understand", "confused"];
    const hasEmotionalSignal = EMOTIONAL_KEYWORDS.some(k => userMessage.toLowerCase().includes(k));
    const isSubstantiveReply = botReply.length > 300;
    return hasEmotionalSignal || isSubstantiveReply;
  };
  ...
  const buildContext = () => {
    ...
    if (listeningStyle === "depth") lines.push(TRACK1_ADDITION);
    if (listeningStyle === "practical") lines.push("\nThis patient prefers connection and practical next steps. Keep responses warm, short, and focused on what to do. Lead with normalization.");
    return lines.length ? `\n\nPATIENT CONTEXT (use to personalize — do not repeat back verbatim):\n${lines.join("\n")}` : "";
  };

  // Check if user message is discussing a risk (informational) vs experiencing an active emergency
  const isActiveEmergency = (text) => {
    const lower = text.toLowerCase();
    const hasRedFlag = RED_FLAG_KEYWORDS.some(k => lower.includes(k));
    if (!hasRedFlag) return false;
    const isInformational = INFORMATIONAL_CONTEXT.some(c => lower.includes(c));
    return !isInformational;
  };

  // Detect style preference from patient's response to the check-in
  const detectStyleChoice = (text) => {
    const lower = text.toLowerCase();
    const depthSignals = ["detail", "research", "science", "data", "statistics", "stats", "numbers", "evidence", "study", "studies", "more detail", "understand the science", "depth"];
    const practicalSignals = ["practical", "simple", "keep it simple", "short", "what to do", "others find", "helpful", "basic", "easy"];
  ...
  const send = async (text) => {
    ...
    // Detect if this message is a style choice response
    if (hasAskedStyle && !listeningStyle) {
      const choice = detectStyleChoice(text);
      if (choice) setListeningStyle(choice);
    }

    const emergency = isActiveEmergency(text);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 500,
          system: SYSTEM_PROMPT_BASE + buildContext(),
          messages: newMessages,
        }),
      });
      const data = await res.json();
      let reply = data.content?.[0]?.text || "I'm having trouble with that response. Please try again.";

      // Prepend emergency notice if active emergency detected
      if (emergency) {
        reply = "⚠️ These symptoms need attention today. Please contact your healthcare team right away or go to an emergency department if you're unsure. Don't wait.\n\n" + reply;
      }

      // Append style check-in if appropriate
      if (shouldAskStyle(text, reply)) {
        reply = reply + CHECK_IN_MESSAGE;
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      <div style={{ background: C.card, borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
        <div className="flex items-center justify-between p-4" style={{ borderBottom: `1px solid ${C.border}` }}>
          <div>
            <div style={{ color: C.navy, fontWeight: 700, fontSize: 16 }}>Ask RENEW</div>
  ...
  return (
    <div id="print-summary" style={{ fontFamily: "Georgia, serif", maxWidth: 700, margin: "0 auto", padding: 32, color: "#1a2e3b" }}>
      <div style={{ textAlign: "center", marginBottom: 32, borderBottom: "2px solid #2d7d6f", paddingBottom: 16 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: "#2d7d6f" }}>RENEW</div>
      ...
      {primarySymptom && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 14, color: "#1a2e3b" }}>The symptom that bothers me most: <strong>{primarySymptom}</strong></div>
        </div>
      )}
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#2d7d6f", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>2. My Symptom Scores</div>
        {scores.wexner !== undefined && <div style={{ marginBottom: 8, fontSize: 13 }}><strong>Wexner Fecal Incontinence Score:</strong> {scores.wexner}/20 — {wexnerBand(scores.wexner).label}</div>}
        {scores.ods !== undefined && <div style={{ marginBottom: 8, fontSize: 13 }}><strong>ODS Score:</strong> {scores.ods}/31 — {odsBand(scores.ods).label}</div>}
        {scores.wexner === undefined && scores.ods === undefined && <div style={{ color: "#7a8fa6", fontSize: 13 }}>No scoring tools completed yet. Complete them in the 'My Scores' section.</div>}
      </div>
      <div style={{ fontSize: 11, color: "#7a8fa6", textAlign: "center", marginTop: 32, borderTop: "1px solid #dde7ef", paddingTop: 16 }}>
        This document was generated by RENEW, a patient education app from Stanford Colorectal Surgery. It is for educational purposes only and does not constitute medical advice or a medical record.
      </div>
    </div>
  );
};
    ...
    <div style={{ background: C.bg, minHeight: "100vh", maxWidth: 480, margin: "0 auto", position: "relative", fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: C.card, borderBottom: `1px solid ${C.border}`, padding: "12px 16px" }}>
        <div className="flex items-center justify-between">
          ...
          <div style={{ color: C.teal, fontWeight: 800, fontSize: 18 }}>🌿 RENEW</div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200 }}>
          ...
          <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 280, background: C.card, padding: "48px 0 24px", overflowY: "auto" }}>
            <div className="px-6 mb-4">
              <div style={{ color: C.teal, fontWeight: 800, fontSize: 20 }}>🌿 RENEW</div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: C.card, borderTop: `1px solid ${C.border}`, display: "flex", paddingBottom: "env(safe-area-inset-bottom, 0)" }}>
        ...
        <button onClick={() => setChatOpen(true)}
          ...
          <span style={{ fontSize: 10, fontWeight: 700 }}>Ask</span>
        </button>
      </div>
